@php
    $bodyClass = 'about-ir';
    $meta_title = 'Sama Aluminium | 재무정보 | 전자공고 | 감사의견';
    $meta_desc = '감사의견';
@endphp
    ...
    <div class="contents-wrap__section clear ir-board-detail" id="ir_board_info">
        <div class="info">
            ...
            <div class="ir-board-file-box">
                {{-- <a href="/eng/about-us/ir/board/file-download?id={{ $ir_board->id }}" class="btn-download"></a> --}}
                <a href="/eng/about-us/ir/board/file-download?id={{ $ir_board->id }}" class="ir-board-detail__download"><span>{{ $ir_board->pdf_file_name}}</span></a>
            </div>
            @endif
        ...
        <div class="img ir-board-detail__img">
            @if ( $ir_board->img_file_name )
                <a href="/storage/{{ $ir_board->img_file_path }}" title="{{ $ir_board->img_file_name }}" class="ir-img">
                    <img src="/eng/storage/{{ $ir_board->img_file_path }}" width="100%" alt="{{ $ir_board->img_file_name }}"/>
                </a>
            @endif
        </div>
    </div>
